...
router.post('/create-pix', async (req, res) => {
  if (!db) {
    return res.status(503).json({ success: false, error: 'Serviço de banco de dados indisponível' });
  }
  try {
...
router.post('/webhook', async (req, res) => {
  if (!db) {
    // O webhook deve retornar 200 OK para evitar reenvio, mas logar o erro.
    console.error('Webhook recebido, mas serviço de banco de dados indisponível.');
